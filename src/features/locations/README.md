...
| `updateLocationApproach(table, id, approach)` | Admin: updates approach text in Supabase + local cache |

`table` for description is `'sub_regions' | 'crags' | 'walls'`; `table` for approach is `'crags' | 'walls'`.
