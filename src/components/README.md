...
| `SunShadeSummary` | Compact tappable pill showing the current month's sun/shade exposure as icons. **full-sun** → orange `Sun` icon; **partial-shade** → grey `Sun` icon; **full-shade** → grey `Cloud` icon. Shows AM icon + `/` + PM icon when they differ for the current month; single icon when they match. Aspect badge (`N`, `SE`, etc.) shown when present. Grey sun icon when `data` is null or no entry for current month. Props: `data: SunData \| null`, `onClick`. |
| `SunShadeSheet` | Full-screen sheet for viewing or editing sun/shade exposure data. **View mode (all users):** aspect badge (if `showAspect`), By Month / By Season toggle, 12-row month table (read-only `<span>` cells), 4-card season grid. **Edit mode (admin only):** 2×4 aspect grid, 12-row cycling cells (`null → full-sun → partial-shade → full-shade`), apply-to-all bulk fill, Save/Cancel in header. Pass `isEditing={isAdmin}` from the parent — non-admins see view mode automatically. Props: `isOpen`, `data: SunData \| null`, `isEditing`, `showAspect`, `onSave?`, `onClose`. |
